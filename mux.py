...
class Multiplexer:
    def __init__(self, mkvtools_path, movie_path):
        ...
        self.output_path = os.path.join(
            self.parent_dir,
            self.movie_name[: self.ext_index]
            + ".mux"
            + self.movie_name[self.ext_index :],
        )
        self.subtitles = []
    ...
    def mux(self, no_subtitles):
        self.subtitles = get_subtitles(
            self.parent_dir, self.movie_name[: self.ext_index]
        )

        sub_params = f""

        for sub_lang, file in self.subtitles.items():
            if "forced" in sub_lang:  # Don't handle forced subs
                self.failed = True
                self.msg = "Forced subtitle"
                return
            if len(sub_lang) != 2:  # English as default language
                sub_lang = "en"
        ...
        if not self.failed:
            ...
            try:
                subprocess.check_output(
                    self.mkvtools_path + "\\mkvmerge.exe  " + params,
    ...
    for f in all_files:
        if movie_name in f:
            if f.endswith(".srt"):
                ...
                sub_lang = filename[len(movie_name) : filename.rfind(".")][
                    1:
                ]  # get language
                subtitles[sub_lang] = os.path.join(parent_path, f)
...
def get_track_number(mkvtools_path, file):
    ...
    file_info = (
        subprocess.Popen(
            f'{mkvtools_path}\\mkvinfo.exe "{file}"', shell=True, stdout=subprocess.PIPE
        )
        .stdout.read()
        .decode("utf-8")
    )
